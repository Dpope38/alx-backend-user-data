...
class Auth:
    """Template for all authentication system implemeted in this app"""
    def require_auth(self, path: str, excluded_paths: List[str]) -> bool:
        ...
        if not excluded_paths:
            return True
        path = path.rstrip('/')
        # Iterate excluded path
        # Check if path is in excluded path and return false if true
        for excluded_path in excluded_paths:
            # check if given path start with (*)
    ...
    def current_user(self, request=None) -> TypeVar('User'):
        ...
        (defaults to None) and returns a value of type that could be user.
        The purpose
